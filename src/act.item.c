...
* File: act.item.c                                                              A CircleMUD/tbaMUD *
* Description: Object handling routines -- get/drop and container handling.                        *
*                                                                                                  *
...
static void perform_put(struct char_data *ch, struct obj_data *obj, struct obj_data *cont) {
	...
	if ((GET_OBJ_VAL(cont, 0) > 0) && (GET_OBJ_WEIGHT(cont) + GET_OBJ_WEIGHT(obj) >
			GET_OBJ_VAL(cont, 0))) {
		act("$p will not fit in $P.", FALSE, ch, obj, cont, TO_CHAR);
	} else if (OBJ_FLAGGED(obj, ITEM_NODROP) && IN_ROOM(cont) != NOWHERE) {
		act("You cannot get $p out of your hand.", FALSE, ch, obj, NULL, TO_CHAR);
	} else {
		obj_from_char(obj);
		obj_to_obj(obj, cont);
 ...
 *     3) put all <container>
 * The <container> must be in inventory or on ground. All objects to be put into container must be
 * in inventory.
 */
ACMD(do_put) {
	...
	if (!*theobj) {
		...
		send_to_char(ch, "What do you want to put %s in?\r\n", obj_dotmode == FIND_INDIV ? "it" :
				"them");
	} else {
		generic_find(thecont, FIND_OBJ_INV | FIND_OBJ_ROOM, ch, &tmp_char, &cont);
		if (!cont) {
			send_to_char(ch, "You do not see %s %s here.\r\n", AN(thecont), thecont);
		} else if (GET_OBJ_TYPE(cont) != ITEM_CONTAINER) {
			act("$p is not a container.", FALSE, ch, cont, 0, TO_CHAR);
		} else if (OBJVAL_FLAGGED(cont, CONT_CLOSED) && (GET_LEVEL(ch) < LVL_IMMORT ||
				!PRF_FLAGGED(ch, PRF_NOHASSLE))) {
			send_to_char(ch, "You had better open it first!\r\n");
		} else {
			if (obj_dotmode == FIND_INDIV) {	/* put <obj> <container> */
				...
				for (obj = ch->carrying; obj; obj = next_obj) {
					next_obj = obj->next_content;
					if (obj != cont && CAN_SEE_OBJ(ch, obj) && (obj_dotmode == FIND_ALL ||
				...
				if (!found) {
					if (obj_dotmode == FIND_ALL) {
						send_to_char(ch, "You do not seem to have anything to put in it.\r\n");
					} else {
						send_to_char(ch, "You do not seem to have any %ss.\r\n", theobj);
					}
				}
			}
		}
...
static int can_take_obj(struct char_data *ch, struct obj_data *obj) {
	if (!(CAN_WEAR(obj, ITEM_WEAR_TAKE))) {
		act("$p: you cannot take that!", FALSE, ch, obj, 0, TO_CHAR);
		return (0);
	}

	if (!IS_NPC(ch) && !PRF_FLAGGED(ch, PRF_NOHASSLE)) {
		if (IS_CARRYING_N(ch) >= CAN_CARRY_N(ch)) {
			act("$p: you cannot carry that many items.", FALSE, ch, obj, 0, TO_CHAR);
			return (0);
		} else if ((IS_CARRYING_W(ch) + GET_OBJ_WEIGHT(obj)) > CAN_CARRY_W(ch)) {
			act("$p: you cannot carry that much weight.", FALSE, ch, obj, 0, TO_CHAR);
			return (0);
		}
	}
...
static void get_check_money(struct char_data *ch, struct obj_data *obj) {
	...
	if ((PRF_FLAGGED(ch, PRF_AUTOSPLIT) && IS_AFFECTED(ch, AFF_GROUP))) {
		...
		if (IS_AFFECTED(k, AFF_GROUP) && (k->in_room == ch->in_room) && !(IS_NPC(k)) && k != ch) {
			...
			send_to_char(ch, "%s splits %ld coins;  you receive %d.\r\n", GET_NAME(ch), value,
					share);
		}
		for (f = k->followers; f; f = f-> next) {
			if (IS_AFFECTED(f->follower, AFF_GROUP) && (!IS_NPC(f->follower)) &&
				...
				send_to_char(f->follower, "%s splits %ld coins; you receive %d.\r\n", GET_NAME(ch),
						value, share);
			}
		}
		send_to_char(ch, "You split %ld coins among %d members;  %d coins each.\r\n", value, num,
				share);
	}
}
...
static void perform_get_from_container(struct char_data *ch, struct obj_data *obj,
	...
	if (mode == FIND_OBJ_INV || can_take_obj(ch, obj)) {
		if (IS_CARRYING_N(ch) >= CAN_CARRY_N(ch)) {
			act("$p: you cannot hold any more items.", FALSE, ch, obj, 0, TO_CHAR);
...
void get_from_container(struct char_data *ch, struct obj_data *cont, char *arg, int mode,
	...
	if (OBJVAL_FLAGGED(cont, CONT_CLOSED) && (GET_LEVEL(ch) < LVL_IMMORT ||
			!PRF_FLAGGED(ch, PRF_NOHASSLE))) {
		...
		if (!(obj = get_obj_in_list_vis(ch, arg, NULL, cont->contains))) {
			...
			snprintf(buf, sizeof(buf), "There does not seem to be %s %s in $p.", AN(arg), arg);
		...
		if (!found) {
			if (obj_dotmode == FIND_ALL) {
				...
				snprintf(buf, sizeof(buf), "You cannot seem to find any %ss in $p.", arg);
				act(buf, FALSE, ch, cont, 0, TO_CHAR);
			}
		}
	}
...
static void get_from_room(struct char_data *ch, char *arg, int howmany) {
	...
	if (dotmode == FIND_INDIV) {
		...
		if (!found) {
			if (dotmode == FIND_ALL) {
				send_to_char(ch, "There does not seem to be anything here.\r\n");
			} else {
				send_to_char(ch, "You do not see any %ss here.\r\n", arg);
			}
		}
	}
}
...
ACMD(do_get) {
	...
	if (IS_CARRYING_N(ch) >= CAN_CARRY_N(ch)) {
		...
		if (cont_dotmode == FIND_INDIV) {
			...
			if (!cont) {
				send_to_char(ch, "You do not have %s %s.\r\n", AN(arg2), arg2);
			} else if (GET_OBJ_TYPE(cont) != ITEM_CONTAINER) {
				act("$p is not a container.", FALSE, ch, cont, 0, TO_CHAR);
			} else {
			...
			for (cont = ch->carrying; cont; cont = cont->next_content) {
				if (CAN_SEE_OBJ(ch, cont) && (cont_dotmode == FIND_ALL ||
						isname(arg2, cont->name))) {
			...
			if (!found) {
				if (cont_dotmode == FIND_ALL) {
					send_to_char(ch, "You cannot seem to find any containers.\r\n");
				} else {
					send_to_char(ch, "You cannot seem to find any %ss here.\r\n", arg2);
				}
			}
		}
	}
...
static void perform_drop_gold(struct char_data *ch, int amount, byte mode, room_rnum RDR) {
	...
	if (amount <= 0) {
		...
		send_to_char(ch, "You do not have that many coins!\r\n");
	} else {
		if (mode != SCMD_JUNK) {
...
static int perform_drop(struct char_data *ch, struct obj_data *obj, byte mode, const char *sname,
	...
	if (OBJ_FLAGGED(obj, ITEM_NODROP) && !PRF_FLAGGED(ch, PRF_NOHASSLE)) {
		snprintf(buf, sizeof(buf), "You cannot %s $p, it must be CURSED!", sname);
...
ACMD(do_drop) {
	...
	switch (subcmd) {
		...
		case SCMD_DONATE:
			...
			if (RDR == NOWHERE) {
				send_to_char(ch, "Sorry, you cannot donate anything right now.\r\n");
				return;
			}
			break;
		default:
	...
	if (!*arg) {
		...
		if (dotmode == FIND_ALL) {
			if (!ch->carrying) {
				send_to_char(ch, "You do not seem to be carrying anything.\r\n");
			} else {
				for (obj = ch->carrying; obj; obj = next_obj) {
					next_obj = obj->next_content;
					amount += perform_drop(ch, obj, mode, sname, RDR);
...
static void perform_give(struct char_data *ch, struct char_data *vict, struct obj_data *obj) {
	...
	if (OBJ_FLAGGED(obj, ITEM_NODROP) && !PRF_FLAGGED(ch, PRF_NOHASSLE)) {
		act("You cannot let go of $p!!  Yeech!", FALSE, ch, obj, 0, TO_CHAR);
		return;
	}
	...
	if (GET_OBJ_WEIGHT(obj) + IS_CARRYING_W(vict) > CAN_CARRY_W(vict) &&
		...
		act("$E cannot carry that much weight.", FALSE, ch, 0, vict, TO_CHAR);
		return;
	}
	obj_from_char(obj);
...
ACMD(do_give) {
	...
	if (!*arg) {
		...
		if (dotmode == FIND_INDIV) {
			if (!(obj = get_obj_in_list_vis(ch, arg, NULL, ch->carrying))) {
				send_to_char(ch, "You do not seem to have %s %s.\r\n", AN(arg), arg);
			} else {
				perform_give(ch, vict, obj);
			}
		} else {
			...
			if (!ch->carrying) {
				send_to_char(ch, "You do not seem to be holding anything.\r\n");
			} else {
				for (obj = ch->carrying; obj; obj = next_obj) {
					next_obj = obj->next_content;
					if (CAN_SEE_OBJ(ch, obj) && ((dotmode == FIND_ALL || isname(arg, obj->name)))) {
...
void name_from_drinkcon(struct obj_data *obj) {
	...
	if (!isname(liqname, obj->name)) {
		log("SYSERR: Cannot remove liquid '%s' from '%s' (%llu) item.", liqname, obj->name,
				obj->item_number);
...
ACMD(do_drink) {
		...
		switch (SECT(IN_ROOM(ch))) {
			...
			case SECT_UNDERWATER:
				if ((GET_COND(ch, HUNGER) > 20) && (GET_COND(ch, THIRST) > 0)) {
					send_to_char(ch, "Your stomach cannot contain anymore!\r\n");
				}
				...
				if (GET_COND(ch, THIRST) > 20) {
					send_to_char(ch, "You do not feel thirsty any more.\r\n");
				}
				return;
			default:
	...
	if (!(temp = get_obj_in_list_vis(ch, arg, NULL, ch->carrying))) {
		if (!(temp = get_obj_in_list_vis(ch, arg, NULL, world[IN_ROOM(ch)].contents))) {
			send_to_char(ch, "You cannot find it!\r\n");
			return;
		} else {
			on_ground = 1;
		}
	...
	if ((GET_OBJ_TYPE(temp) != ITEM_DRINKCON) && (GET_OBJ_TYPE(temp) != ITEM_FOUNTAIN)) {
		send_to_char(ch, "You cannot drink from that!\r\n");
		return;
	}
	...
	if ((GET_COND(ch, DRUNK) > 10) && (GET_COND(ch, THIRST) > 0)) {
		/* The pig is drunk */
		send_to_char(ch, "You cannot seem to get close enough to your mouth.\r\n");
...
ACMD(do_eat) {
	...
	if ((GET_OBJ_TYPE(food) != ITEM_FOOD) && (GET_LEVEL(ch) < LVL_IMMORT)) {
		send_to_char(ch, "You cannot eat THAT!\r\n");
		return;
	}
	...
	if (subcmd == SCMD_EAT) {
		...
		if (!(--GET_OBJ_VAL(food, 0))) {
			send_to_char(ch, "There is nothing left now.\r\n");
			extract_obj(food);
		}
	}
}
...
ACMD(do_pour) {
	...
	if (subcmd == SCMD_POUR) {
		...
		if (GET_OBJ_TYPE(from_obj) != ITEM_DRINKCON) {
			send_to_char(ch, "You cannot pour from that!\r\n");
			return;
		}
	}
	if (subcmd == SCMD_FILL) {
		...
		if (GET_OBJ_TYPE(to_obj) != ITEM_DRINKCON) {
			act("You cannot fill $p!", FALSE, ch, to_obj, 0, TO_CHAR);
			return;
		}
		...
		if (GET_OBJ_TYPE(from_obj) != ITEM_FOUNTAIN) {
			act("You cannot fill something from $p.", FALSE, ch, from_obj, 0, TO_CHAR);
			return;
		}
	}
	...
	if (subcmd == SCMD_POUR) { /* pour */
		...
		if (!str_cmp(arg2, "out")) {
			if (GET_OBJ_VAL(from_obj, 0) > 0) {
				...
				send_to_char(ch, "You cannot possibly pour that container out!\r\n");
			}
			return;
		}
		...
		if ((GET_OBJ_TYPE(to_obj) != ITEM_DRINKCON) && (GET_OBJ_TYPE(to_obj) != ITEM_FOUNTAIN)) {
			send_to_char(ch, "You cannot pour anything into that.\r\n");
			return;
		}
	}
	if (to_obj == from_obj) {
...
static void wear_message(struct char_data *ch, struct obj_data *obj, int where) {
	const char *wear_messages[][2] = {
		...
		{"$n puts $p on $s back.", "You put $p on your back."},
		{"$n puts $p on $s face.", "You put $p on your face."},
		{"$n dual-wields $p.", "You dual-wield $p."}
	};
...
void perform_wear(struct char_data *ch, struct obj_data *obj, int where) {
	...
	int wear_bitvectors[] = {
		ITEM_WEAR_TAKE, ITEM_WEAR_FINGER, ITEM_WEAR_FINGER, ITEM_WEAR_NECK, ITEM_WEAR_NECK,
		...
		ITEM_WEAR_WRIST, ITEM_WEAR_WIELD, ITEM_WEAR_TAKE, ITEM_WEAR_BACK, ITEM_WEAR_FACE,
		ITEM_WEAR_DUALWIELD
	};
	struct obj_data *obj2 = GET_EQ(ch, WEAR_WIELD), *obj3 = GET_EQ(ch, WEAR_DUALWIELD);
	const char *already_wearing[] = {
		"You are already using a light.\r\n",
		"YOU SHOULD NEVER SEE THIS MESSAGE.  PLEASE REPORT.\r\n",
		"You are already wearing something on both of your ring fingers.\r\n",
		"YOU SHOULD NEVER SEE THIS MESSAGE.  PLEASE REPORT.\r\n",
		"You cannot wear anything else around your neck.\r\n",
		"You are already wearing something on your body.\r\n",
		...
		"You are already wearing something on your hands.\r\n",
		"You are already wearing something on your arms.\r\n",
		"You are already using a shield.\r\n",
		"You are already wearing something about your body.\r\n",
		"You already have something around your waist.\r\n",
		"YOU SHOULD NEVER SEE THIS MESSAGE.  PLEASE REPORT.\r\n",
		"You are already wearing something around both of your wrists.\r\n",
		...
		"You are already dual-wielding a weapon.\r\n"
	};

	/* first, make sure that the wear position is valid. */
	if (!CAN_WEAR(obj, wear_bitvectors[where])) {
		act("You cannot wear $p there.", FALSE, ch, obj, 0, TO_CHAR);
		return;
	}
	...
	if (((obj2 && OBJ_FLAGGED(obj2, ITEM_2HANDED)) || obj3) && where == WEAR_SHIELD) {
		send_to_char(ch, "You do not have a free hand to wield a shield.\r\n");
		return;
	}
...
ACMD(do_wear) {
	...
	if (*arg2 && (dotmode != FIND_INDIV)) {
		send_to_char(ch, "You cannot specify the same body location for more than one item!\r\n");
		return;
	}
	if (dotmode == FIND_ALL) {
		for (obj = ch->carrying; obj; obj = next_obj) {
		...
		if (!items_worn) {
			send_to_char(ch, "You do not seem to have anything wearable.\r\n");
		}
	} else if (dotmode == FIND_ALLDOT) {
		if (!*arg1) {
...
ACMD(do_wield) {
	char arg[MAX_INPUT_LENGTH];
	struct obj_data *obj,
			*obj2 = GET_EQ(ch, WEAR_HOLD),
			*obj3 = GET_EQ(ch, WEAR_DUALWIELD),
			*obj4 = GET_EQ(ch, WEAR_SHIELD);
	...
	if (!*arg) {
		...
		send_to_char(ch, "You do not seem to have %s %s.\r\n", AN(arg), arg);
  	} else if ((OBJ_FLAGGED(obj, ITEM_ENGRAVED) && strcmp(obj->owner_name, GET_NAME(ch)))) {
		send_to_char(ch, "A strange force seems to stop you from wielding that!\r\n");
	} else {
		if (!CAN_WEAR(obj, ITEM_WEAR_WIELD)) {
			send_to_char(ch, "You cannot wield that.\r\n");
		} else if (GET_OBJ_WEIGHT(obj) > str_app[STRENGTH_APPLY_INDEX(ch)].wield_w) {
			send_to_char(ch, "It is too heavy for you to use.\r\n");
		} else if (GET_LEVEL(ch) < GET_OBJ_LEVEL(obj)) {
			send_to_char(ch, "You are not experienced enough to use that.\r\n");
		} else if (OBJ_FLAGGED(obj, ITEM_2HANDED)) {
			if (obj2 || obj3 || obj4) {
				send_to_char(ch, "You need both hands free to wield that.\r\n");
			} else {
...
ACMD(do_dualwield) {
	char arg[MAX_INPUT_LENGTH];
	struct obj_data *obj,
			*obj2 = GET_EQ(ch, WEAR_WIELD),
			*obj3 = GET_EQ(ch, WEAR_HOLD),
			*obj4 = GET_EQ(ch, WEAR_SHIELD);

	if (IS_RANGER(ch) || (IS_WARRIOR(ch) && GET_LEVEL(ch) >= 65) || IS_DARK_ELF(ch)) {
		one_argument(argument, arg);
		if (!*arg) {
			...
			send_to_char(ch, "A strange force seems to stop you from dual-wielding that!\r\n");
		} else {
			if (!CAN_WEAR(obj, ITEM_WEAR_DUALWIELD)) {
				send_to_char(ch, "You cannot dual-wield that.\r\n");
			} else if (GET_OBJ_WEIGHT(obj) > str_app[STRENGTH_APPLY_INDEX(ch)].wield_w) {
				send_to_char(ch, "It is too heavy for you to use.\r\n");
			} else if (obj2 && OBJ_FLAGGED(obj2, ITEM_2HANDED)) {
				send_to_char(ch, "You are wielding a two-handed weapon and do not have a free hand.\r\n");
			} else if (obj3) {
				send_to_char(ch, "You are already holding something in your other hand.\r\n");
			} else if (obj4) {
				send_to_char(ch, "You are wielding a shield in your other hand.\r\n");
			} else {
				perform_wear(ch, obj, WEAR_DUALWIELD);
			}
		}
	}
}
	...
	if (!*arg) {
		...
		if (GET_OBJ_TYPE(obj) == ITEM_LIGHT) {
			...
			if (!CAN_WEAR(obj, ITEM_WEAR_HOLD) && GET_OBJ_TYPE(obj) != ITEM_WAND &&
				...
				send_to_char(ch, "You cannot hold that.\r\n");
			} else if (obj2 && OBJ_FLAGGED(obj2, ITEM_2HANDED)) {
				send_to_char(ch, "You are wielding a two-handed weapon and do not have a free hand.\r\n");
			} else if (obj3) {
				send_to_char(ch, "You are already dual-wielding something in your other hand.\r\n");
			} else if (obj4) {
				send_to_char(ch, "You are wielding a shield in your other hand.\r\n");
			} else {
...
static void perform_remove(struct char_data *ch, int pos) {
	...
	if (!(obj = GET_EQ(ch, pos))) {
		...
		/*
		 * This error occurs when perform_remove() is passed a bad 'pos' (location) to remove an
		 * object from.
		 */
	} else if (OBJ_FLAGGED(obj, ITEM_NODROP) && !PRF_FLAGGED(ch, PRF_NOHASSLE)) {
		act("You cannot remove $p, it must be CURSED!", FALSE, ch, obj, 0, TO_CHAR);
...
ACMD(do_remove) {
	...
	if (dotmode == FIND_ALL) {
		...
		if (!found) {
			send_to_char(ch, "You are not using anything.\r\n");
		}
	} else if (dotmode == FIND_ALLDOT) {
		if (!*arg) {
			...
			if (!found) {
				send_to_char(ch, "You do not seem to be using any %ss.\r\n", arg);
			}
		}
	} else {
		if ((i = get_obj_pos_in_equip_vis(ch, arg, NULL, ch->equipment)) < 0) {
			send_to_char(ch, "You do not seem to be using %s %s.\r\n", AN(arg), arg);
		} else {
			perform_remove(ch, i);
		}
	}
...
ACMD(do_sac) {
	...
	if (!(j = get_obj_in_list_vis(ch, arg, NULL, world[IN_ROOM(ch)].contents)) &&
		...
		send_to_char(ch, "It does not seem to be here.\r\n");
		return;
	}
	...
	switch (rand_number(0, 5)) {
		...
		case 4:
			send_to_char(ch,
					"Your sacrifice to the Gods is rewarded with %d gold coins.\r\n",
					(1 + GET_OBJ_LEVEL(j)));
...
ACMD(do_repair) {
	...
	if (GET_SKILL(ch, SKILL_REPAIR) <= 0) {
		send_to_char(ch, "You do not know how to repair things!\r\n");
		return;
	}
	one_argument(argument, arg);
	if (!*arg) {
	...
	if (!(repair = get_obj_in_list_vis(ch, arg, NULL, ch->carrying))) {
		send_to_char(ch, "You do not seem to have %s %s.\r\n", AN(arg), arg);
		return;
	}
	...
	if ((GET_OBJ_CSLOTS(repair) == 0) && (GET_OBJ_TSLOTS(repair) == 0)) {
		act("$p seems to be indestructible!", FALSE, ch, repair, 0, TO_CHAR);
		return;
	}
	...
	if (GET_OBJ_CSLOTS(repair) < 0) {
		act("You completely ruin $p and it crumbles away!", FALSE, ch, repair, 0, TO_CHAR);
		act("$n tries to repair $p, but it crumbles away!", TRUE, ch, repair, 0, TO_ROOM);
		extract_obj(repair); return;
	}
	if (percent > prob) {
		act("Your clumsy attempt at repairing $p damages it even more!", FALSE, ch, repair, 0, TO_CHAR);
		act("$n tries to repair $p, but only makes it worse!", TRUE, ch, repair, 0, TO_ROOM);
		GET_OBJ_CSLOTS(repair) -= 2;
		GET_OBJ_TSLOTS(repair) -= 1;
		return;
	} else {
		act("You repair $s and it looks in excellent condition again!",FALSE, ch, repair, 0, TO_CHAR);
		act("$n repairs $p, making it as good as new again!", TRUE, ch, repair, 0, TO_ROOM);
		GET_OBJ_TSLOTS(repair) -= 1;
		GET_OBJ_CSLOTS(repair) = GET_OBJ_TSLOTS(repair);
		SET_SKILL(ch, SKILL_REPAIR, (GET_SKILL(ch,SKILL_REPAIR) + 1));
	}
}
...
ACMD(do_compare) {
	...
	int dotmode, value1, value2;

	two_arguments(argument, arg1, arg2);
	if (!*arg1) {
		send_to_char(ch, "Compare what?\r\n");
		return;
	} else if (!*arg2) {
		send_to_char(ch, "Compare what?\r\n");
		return;
	...
	dotmode = find_all_dots(arg1);
	if (dotmode != FIND_INDIV) {
		send_to_char(ch, "You can only compare two items!\r\n");
		return;
	}
	...
	if (obj1 == obj2) {
		send_to_char(ch, "How about we compare two different items.\r\n");
		return;
	}
	if (GET_OBJ_TYPE(obj1) != GET_OBJ_TYPE(obj2)) {
		send_to_char(ch, "Is that not a lot like comparing apples and oranges?\r\n");
		return;
	}
	if (GET_OBJ_TYPE(obj1) == ITEM_WEAPON || GET_OBJ_TYPE(obj1) == ITEM_FIREWEAPON ||
			GET_OBJ_TYPE(obj1) == ITEM_MISSILE) {
		value1 = (GET_OBJ_VAL(obj1, 1) * GET_OBJ_VAL(obj1, 2) * 100) + GET_OBJ_TSLOTS(obj1);
		value2 = (GET_OBJ_VAL(obj2, 1) * GET_OBJ_VAL(obj2, 2) * 100) + GET_OBJ_TSLOTS(obj2);
	} else {
		value1 = GET_OBJ_COST(obj1);
		value2 = GET_OBJ_COST(obj2);
	}
	if (value1 > value2) {
		send_to_char(ch, "The %s seems better than the %s.\r\n", obj1->name, obj2->name);
	} else if (value1 < value2) {
		send_to_char(ch, "The %s seems better than the %s.\r\n", obj2->name, obj1->name);
	} else {
		send_to_char(ch, "You cannot seem to determine which item is better.\r\n");
	}
}

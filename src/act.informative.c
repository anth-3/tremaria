...
* File: act.informative.c                                                       A CircleMUD/tbaMUD *
* Description: Player-level commands of an informative nature.                                     *
*                                                                                                  *
* All rights reserved.  See license for complete information.                                      *
*                                                                                                  *
* Copyright (C) 1993, 94 by the Trustees of the Johns Hopkins University                           *
* CircleMUD is based on DikuMUD, Copyright (C) 1990, 1991.                                         *
* Last updated: 22 April 2014 -- anth*3                                                            *
***************************************************************************************************/
...
static void show_obj_to_char(struct obj_data *obj, struct char_data *ch, int mode) {
	...
	if (!obj || !ch) {
		...
		/*
		 * SYSERR_DESC: Somehow a NULL pointer was sent to show_obj_to_char() in either the 'obj'
		 * or the 'ch' variable.  The error will indicate which was NULL by listing both of the
		 * pointers passed to it.  This is often a difficult one to trace, and may require stepping
		 * through a debugger.
		 */
		return;
	}
	...
	switch (mode) {
		case SHOW_OBJ_LONG:
			...
			if (!IS_NPC(ch) && PRF_FLAGGED(ch, PRF_SHOWVNUMS)) {
				send_to_char(ch, "[%llu] ", GET_OBJ_VNUM(obj));
				if (SCRIPT(obj)) {
					if (!TRIGGERS(SCRIPT(obj))->next) {
						send_to_char(ch, "[T%llu] ", GET_TRIG_VNUM(TRIGGERS(SCRIPT(obj))));
					} else {
			...
			switch (GET_OBJ_TYPE(obj)) {
				case ITEM_NOTE:
					if (obj->action_description) {
	        			...
	        			if (rand_number(1, 95) > GET_SKILL(ch, GET_OBJ_VAL(obj, 0) + 1512)) {
	        				snprintf(notebuf, sizeof(notebuf),
	        						"%sIt is in an unfamiliar language which you do not understand.\r\n",
	        						notebuf);
	        			} else {
	        				snprintf(notebuf, sizeof(notebuf), "%s%s", notebuf,
		...
		default:
			...
			/*
			 * SYSERR_DESC:  show_obj_to_char() has some predefined 'mode's (argument #3) to tell
			 * it what to display to the character when it is called.  If the mode is not one of
			 * these, it will output this error, and indicate what mode was passed to it.  To
			 * correct it, you will need to find the call with the incorrect mode and change it to
			 * an acceptable mode.
			 */
			return;
	}
...
static void show_obj_modifiers(struct obj_data *obj, struct char_data *ch) {
	...
	if (OBJ_FLAGGED(obj, ITEM_BURIED) && PRF_FLAGGED(ch, PRF_HOLYLIGHT)) {
		send_to_char(ch, " (buried)");
	}
	if (obj) {
	...
	if (condition != 0) {
		if (condition <= 10) {
			send_to_char(ch, " (extremely poor)");
		} else if (condition > 10 && condition <= 20) {
			send_to_char(ch, " (poor)");
		} else if (condition > 20 && condition <= 30) {
			send_to_char(ch, " (fair)");
		} else if (condition > 30 && condition <= 40) {
			send_to_char(ch, " (moderate)");
		} else if (condition > 40 && condition <= 50) {
			send_to_char(ch, " (good)");
		} else if (condition > 50 && condition <= 60) {
			send_to_char(ch, " (very good)");
		} else if (condition > 60 && condition <= 70) {
			send_to_char(ch, " (excellent)");
		} else if (condition > 70 && condition <= 80) {
			send_to_char(ch, " (superior)");
		} else if (condition > 80 && condition <= 90) {
			send_to_char(ch, " (extremely superior)");
		}  else {
			send_to_char(ch, " (good as new)");
		}
	}
}
...
static void look_at_char(struct char_data *i, struct char_data *ch) {
	...
	diag_char_to_char(i, ch);
   	...
   	if (RIDING(i) && RIDING(i)->in_room == i->in_room) {
		if (RIDING(i) == ch) {
			act("$e is riding on you.", FALSE, i, 0, ch, TO_VICT);
		} else {
			sprintf(buf, "$e is riding upon %s.", PERS(RIDING(i), ch));
			act(buf, FALSE, i, 0, ch, TO_VICT);
		}
   	} else if (RIDDEN_BY(i) && RIDDEN_BY(i)->in_room == i->in_room) {
		if (RIDDEN_BY(i) == ch)
			act("You are riding $m.", FALSE, i, 0, ch, TO_VICT);
		else {
			sprintf(buf, "$e is ridden by %s.", PERS(RIDDEN_BY(i), ch));
			act(buf, FALSE, i, 0, ch, TO_VICT);
		}
   	}
...
static void list_one_char(struct char_data *i, struct char_data *ch) {
	...
	if (!IS_NPC(ch) && PRF_FLAGGED(ch, PRF_SHOWVNUMS)) {
		if (IS_NPC(i))
			send_to_char(ch, "[%llu] ", GET_MOB_VNUM(i));
		if (SCRIPT(i) && TRIGGERS(SCRIPT(i))) {
			if (!TRIGGERS(SCRIPT(i))->next)
				send_to_char(ch, "[T%llu] ", GET_TRIG_VNUM(TRIGGERS(SCRIPT(i))));
			else
				send_to_char(ch, "[TRIGS] ");
		}
	}
	...
	if (RIDING(i) && RIDING(i)->in_room == i->in_room) {
		send_to_char(ch, " is here, riding %s.", (RIDING(i) == ch ? "you" : PERS(RIDING(i), ch)));
...
static void list_char_to_char(struct char_data *list, struct char_data *ch) {
	...
	for (i = list; i; i = i->next_in_room) {
		if (ch != i) {
			 ...
			 */
			if (!IS_NPC(ch) && !PRF_FLAGGED(ch, PRF_HOLYLIGHT) && IS_NPC(i) &&
...
static void do_auto_exits(struct char_data *ch) {
	...
	for (door = 0; door < DIR_COUNT; door++) {
		...
		if (SECT(EXIT(ch, door)->to_room) == SECT_WALL && SECT(ch->in_room) != SECT_WALL) {
			continue;
		}
		if (EXIT_FLAGGED(EXIT(ch, door), EX_CLOSED)) {
			send_to_char(ch, "%s(%s)%s ", EXIT_FLAGGED(EXIT(ch, door),
					EX_HIDDEN) ? CCWHT(ch, C_NRM) : CCRED(ch, C_NRM), autoexits[door],
					CCCYN(ch, C_NRM));
	...
	if (world[ch->in_room].func == impl_room) {
		send_to_char(ch, "%s%s%s", CCRED(ch, C_NRM), "Up    - The Entrance to Another Place\r\n",
				CCCYN(ch, C_NRM));
		send_to_char(ch, "%s%s%s", CCRED(ch, C_NRM), "Down  - The Entrance to the Morgue\r\n",
				CCCYN(ch, C_NRM));
		slen++;
	}
	if (world[ch->in_room].func == morgue_room) {
		send_to_char(ch, "%s%s%s", CCRED(ch, C_NRM),
				"Up    - The Entrance to the Temple of the Implementors", CCCYN(ch, C_NRM));
		slen++;
	}
	if (world[ch->in_room].func == newbie_room) {
		send_to_char(ch, "%s%s%s", CCRED(ch, C_NRM),
				"Down  - The Entrance to the Temple of the Implementors", CCCYN(ch, C_NRM));
		slen++;
	}
...
ACMD(do_exits) {
	...
	if (AFF_FLAGGED(ch, AFF_BLIND) && GET_LEVEL(ch) < LVL_IMMORT) {
		send_to_char(ch, "You cannot see a damned thing, you are blind!\r\n");
		return;
	}
	...
	if (world[ch->in_room].func == impl_room) {
		if (GET_LEVEL(ch) >= LVL_IMMORT) {
			send_to_char(ch, "Up    - [20003] The Entrance to Another Place\r\n");
			send_to_char(ch, "Down  - [20000] The Entrance to the Morgue\r\n");
		} else {
			send_to_char(ch, "Up    - The Entrance to Another Place\r\n");
			send_to_char(ch, "Down  - The Entrance to the Morgue\r\n");
		}
		len++;
	}
	if (world[ch->in_room].func == morgue_room) {
		if (GET_LEVEL(ch) >= LVL_IMMORT) {
			send_to_char(ch, "Up    - [Shrug] The Entrance to the Temple of the Implementors\r\n");
		} else {
			send_to_char(ch, "Up    - The Entrance to the Temple of the Implementors\r\n");
		}
		len++;
	}
	if (world[ch->in_room].func == newbie_room) {
		if (GET_LEVEL(ch) >= LVL_IMMORT) {
			send_to_char(ch, "Down  - [Shrug] The Entrance to the Temple of the Implementors\r\n");
		} else {
			send_to_char(ch, "Down  - The Entrance to the Temple of the Implementors\r\n");
		}
		len++;
	}
	if (!len) {
...
void look_at_room(struct char_data *ch, int ignore_brief) {
	...
	char *blood_messages[] = {
  		...
  		"You are in some blood.",
  		"The blood in this room is flowing.",
  		"There is so much blood here it is intoxicating!",
	...
	char *snow_messages[] = {
  		"Should never see this.",
  		"There is a light dusting of snow here.",
  		"You are in an inch of snow.",
  		"There are a couple of inches of snow here.",
  		"There are quite a few inches of snow here.",
  		"There is at least half a foot of snow here.",
  		"You are standing in a foot of snow.",
  		"The snow here comes up to your knees!",
  		"You are in snow up to your thighs!",
  		"There are some huge snow drifts here, with the snow to your waist!",
  		"There is snow much snow here, you can hardly move!",
  		"\n"
	};
	...
	if (!IS_NPC(ch) && PRF_FLAGGED(ch, PRF_SHOWVNUMS)) {
		...
		send_to_char(ch, "[%5llu] ", GET_ROOM_VNUM(IN_ROOM(ch)));
		...
		if (SCRIPT(rm)) {
			...
			for (t = TRIGGERS(SCRIPT(rm)); t; t = t->next) {
				send_to_char(ch, " %llu", GET_TRIG_VNUM(t));
			}
			send_to_char(ch, "]");
		}
	} else {
	...
	if (SECT(ch->in_room) != SECT_INSIDE) {
		/*magenta for dark, green for twilight/dawn, yellow for day */
	...
	send_to_char(ch, "%s", CCNRM(ch, C_NRM));
	if (ROOM_BLOOD(ch->in_room) > 0) {
		send_to_char(ch, "%s", CCRED(ch, C_NRM));
		if (ROOM_BLOOD(ch->in_room) <= 100) {
			act(blood_messages[(ROOM_BLOOD(ch->in_room) / 10) + 1], FALSE, ch, 0, 0, TO_CHAR);
		} else {
			act(blood_messages[10], FALSE, ch, 0, 0, TO_CHAR);
		}
		send_to_char(ch, "%s", CCNRM(ch, C_NRM));
	}
	if (ROOM_SNOW(ch->in_room) > 0) {
		send_to_char(ch, "%s", CCCYN(ch, C_NRM));
		if (ROOM_SNOW(ch->in_room) <= 100) {
			act(snow_messages[(ROOM_SNOW(ch->in_room) / 10) + 1], FALSE, ch, 0, 0, TO_CHAR);
		} else {
			act(snow_messages[10], FALSE, ch, 0, 0, TO_CHAR);
		}
		send_to_char(ch, "%s", CCNRM(ch, C_NRM));
	}
...
static void look_in_obj(struct char_data *ch, char *arg) {
	...
	if (!*arg) {
		send_to_char(ch, "Look in what?\r\n");
			...
			&obj))) {
		send_to_char(ch, "There does not seem to be %s %s here.\r\n", AN(arg), arg);
	} else if ((GET_OBJ_TYPE(obj) != ITEM_DRINKCON) && (GET_OBJ_TYPE(obj) != ITEM_FOUNTAIN) &&
			(GET_OBJ_TYPE(obj) != ITEM_CONTAINER)) {
		send_to_char(ch, "There is nothing inside that!\r\n");
	} else {
		if (GET_OBJ_TYPE(obj) == ITEM_CONTAINER) {
			if (OBJVAL_FLAGGED(obj, CONT_CLOSED) && (GET_LEVEL(ch) < LVL_IMMORT ||
			...
			if ((GET_OBJ_VAL(obj, 1) == 0) && (GET_OBJ_VAL(obj, 0) != -1)) {
				...
				if (GET_OBJ_VAL(obj, 0) < 0) {
					...
					send_to_char(ch, "It is full of a %s liquid.\r\n", buf2);
					...
					sprinttype(GET_OBJ_VAL(obj, 2), color_liquid, buf2, sizeof(buf2));
					send_to_char(ch, "It is %sfull of a %s liquid.\r\n", fullness[amt], buf2);
				}
			}
		}
	}
...
ACMD(do_look) {
	...
	if (GET_POS(ch) < POS_SLEEPING) {
		send_to_char(ch, "You cannot see anything but stars!\r\n");
	} else if (AFF_FLAGGED(ch, AFF_BLIND) && GET_LEVEL(ch) < LVL_IMMORT) {
		send_to_char(ch, "You cannot see a damned thing, you are blind!\r\n");
	} else if (IS_DARK(IN_ROOM(ch)) && !CAN_SEE_IN_DARK(ch)) {
		send_to_char(ch, "It is pitch black...\r\n");
		...
		/* did the char type 'look <direction>?' */
			...
			if (!found) {
				send_to_char(ch, "You could not find anything noticeable.\r\n");
			}
		} else {
			look_at_target(ch, arg);
		}
...
ACMD(do_score) {
	...
	send_to_char(ch, "You are a %d years old %s %s.", GET_AGE(ch), pc_race_types[(int)GET_RACE(ch)],
			pc_class_types[(int)GET_CLASS(ch)]);

	if (age(ch)->month == 0 && age(ch)->day == 0) {
		send_to_char(ch, "  It is your birthday today.\r\n");
	} else {
		send_to_char(ch, "\r\n");
	}

	send_to_char(ch, "You are %d cm in height and weigh %d kg.\r\n", GET_HEIGHT(ch), GET_WEIGHT(ch));

	send_to_char(ch, "Str: %s%d/%d%s\r\n"
					 "Int: %s%d%s\t\t\tHP: %d(%d)\r\n"
					 "Wis: %s%d%s\t\t\tMP: %d(%d)\r\n"
					 "Dex: %s%d%s\t\t\tMV: %d(%d)\r\n"
					 "Con: %s%d%s\r\n"
					 ...
					 CCCYN(ch, C_NRM), GET_INT(ch), CCNRM(ch, C_NRM), GET_HIT(ch), GET_MAX_HIT(ch),
					 CCCYN(ch, C_NRM), GET_WIS(ch), CCNRM(ch, C_NRM), GET_MANA(ch), GET_MAX_MANA(ch),
					 CCCYN(ch, C_NRM), GET_DEX(ch), CCNRM(ch, C_NRM), GET_MOVE(ch), GET_MAX_MOVE(ch),
					 CCCYN(ch, C_NRM), GET_CON(ch), CCNRM(ch, C_NRM),
					 CCCYN(ch, C_NRM), GET_CHA(ch), CCNRM(ch, C_NRM), compute_armor_class(ch));

	send_to_char(ch, "\t\t\tExp: %d\r\n"
					 "Gold: %s%ld%s\r\n"
					 "Align: %d\r\n"
					 "Deaths: %s%d%s\t\t\tKills: %s%d/%ld%s (P/M)\r\n"
					 "Carried weight: %d of %d\r\n"
					 "Carried items: %d of %d",
					 GET_EXP(ch),
					 CCYEL(ch, C_NRM), GET_GOLD(ch), CCNRM(ch, C_NRM),
					 GET_ALIGNMENT(ch),
					 CCRED(ch, C_NRM), ch->player_specials->saved.died, CCNRM(ch, C_NRM),
					 CCCYN(ch, C_NRM), ch->player_specials->saved.play_killed,
					 ch->player_specials->saved.mob_killed, CCNRM(ch, C_NRM),
	...
	switch (GET_POS(ch)) {
		...
		case POS_INCAP:
			...
			break;
		case POS_STUNNED:
			send_to_char(ch, "You are stunned!  You cannot move!\r\n");
			break;
		case POS_SLEEPING:
	...
	if (GET_LEVEL(ch) >= LVL_IMMORT) {
		...
		send_to_char(ch, "Your current zone: %s%llu%s\r\n", CCCYN(ch, C_NRM), GET_OLC_ZONE(ch),
				CCNRM(ch, C_NRM));
	}
}
...
char *WHO_USAGE =
	"Usage: who [minlev[-maxlev]] [-n name] [-r races] [-c classes] [-qimgklozs]\r\n"
	"\r\n"
	" Races: \r\n"
	"  A.] Human      G.] Dark-Dwarf   M.] Lizardman\r\n"
	"  B.] Half-Elf   H.] Halfling     N.] Sahuagin\r\n"
	"  C.] Elf        I.] Gnome        O.] Kestryl\r\n"
	"  D.] Dark-Elf   J.] Dark-Gnome   P.] Centaur\r\n"
	"  E.] Sea-Elf    K.] Half-Giant\r\n"
	"  F.] Dwarf      L.] Minotaur\r\n"
	"\r\n"
	" Classes: \r\n"
	"  A.] Mage          E.] Warrior        I.] Monk\r\n"
	"  B.] Necromancer   F.] Paladin        J.] Bard\r\n"
	"  C.] Cleric        G.] Anti-Paladin   K.] Thief\r\n"
	"  D.] Druid         H.] Ranger\r\n"
	"\r\n"
	" Switches: \r\n"
	"  -q = show questers\r\n"
...
/* Written by Rhade/Daniel Koepke [aka Argyle macleod] of the Keep */
ACMD(do_who) {
	...
	int showclass = 0, short_list = 0, outlaws = 0, showimmort = 0, showmort = 0;
	int who_room = 0, showrace = 0, showgroup = 0, showleader = 0;
	...
	for (d = descriptor_list; d && !short_list; d = d->next) {
		...
		if (CAN_SEE(ch, tch) && IS_PLAYING(d)) {
			...
			if ((showimmort && GET_LEVEL(tch) < LVL_IMMORT) || (showmort &&
		...
		for (d = descriptor_list; d; d = d->next) {
			...
			if (short_list) {
				send_to_char(ch, "%s[%2d %s %s] %-12.12s%s%s",
						(GET_LEVEL(tch) >= LVL_IMMORT ? CCYEL(ch, C_SPR) : ""), GET_LEVEL(tch),
						RACE_ABBR(tch), CLASS_ABBR(tch), GET_NAME(tch), CCNRM(ch, C_SPR),
						((!(++num_can_see % 4)) ? "\r\n" : ""));
			} else {
				num_can_see++;
				send_to_char(ch, "%s[%2d %s %s] %s%s%s%s",
						(GET_LEVEL(tch) >= LVL_IMMORT ? CCYEL(ch, C_SPR) : ""), GET_LEVEL(tch),
						RACE_ABBR(tch), CLASS_ABBR(tch), GET_NAME(tch), (*GET_TITLE(tch) ? " " : ""),
				...
				if (PRF_FLAGGED(tch, PRF_BOUNTYHUNT)) {
					send_to_char(ch, " (BNTY)");
				}
				if (PRF_FLAGGED(tch, PRF_ASSASSIN)) {
					send_to_char(ch, " (ASSN");
				}
				send_to_char(ch, "\r\n");
			}
		}
	...
	send_to_char(ch, "There is a boot time high of %d player%s.", boot_high,
			(boot_high == 1 ? "" : "s"));
	if (IS_HAPPYHOUR > 0) {
...
ACMD(do_users) {
	...
	send_to_char(ch,
		"Num Race   Class   Name         State          Idl   Login\t*   Site\r\n"
		"--- ----- ------- ------------ -------------- ----- -------- ------------------------\r\n");
	...
	for (d = descriptor_list; d; d = d->next) {
		...
		*(timeptr + 8) = '\0';
		...
		if (d->character && STATE(d) == CON_PLAYING) {
			sprintf(idletime, "%5d", d->character->char_specials.timer * SECS_PER_MUD_HOUR /
					SECS_PER_REAL_MIN);
		} else {
			strcpy(idletime, "     ");
		}
	...
	if (!*arg) {
		...
		for (d = descriptor_list; d; d = d->next) {
			if (IS_PLAYING(d)) {
				...
				if (i && CAN_SEE(ch, i) && (IN_ROOM(i) != NOWHERE)) {
					if (d->original) {
						send_to_char(ch, "%-8s%s - [%5llu] %s%s (in %s%s)\r\n", GET_NAME(i), QNRM,
								GET_ROOM_VNUM(IN_ROOM(d->character)),
								world[IN_ROOM(d->character)].name, QNRM, GET_NAME(d->character),
								QNRM);
					} else {
						send_to_char(ch, "%-8s%s %s[%s%5llu%s]%s %-*s%s %s%s\r\n", GET_NAME(i),
								QNRM, QCYN, QYEL, GET_ROOM_VNUM(IN_ROOM(i)), QCYN, QNRM,
		...
		for (i = character_list; i; i = i->next) {
			if (CAN_SEE(ch, i) && IN_ROOM(i) != NOWHERE && isname(arg, i->player.name)) {
				found = 1;
				send_to_char(ch, "M%3d. %-25s%s - [%5llu] %-25s%s", ++num, GET_NAME(i), QNRM,
		...
		if (!found) {
			send_to_char(ch, "Could not find any such thing.\r\n");
		}
	}
}
...
ACMD(do_levels) {
	...
	if (IS_NPC(ch)) {
		send_to_char(ch, "You are in no position to do that.\r\n");
		return;
	}
	one_argument(argument, arg);
		...
		switch (GET_SEX(ch)) {
			...
			default:
				nlen = snprintf(buf + len, sizeof(buf) - len,
						"Oh dear.  You seem to be sexless.\r\n");
				break;
		}
		if (len + nlen >= sizeof(buf)) {
			break;
...
extern int thaco(int class_num, int level);

ACMD(do_consider) {
	char buf[MAX_INPUT_LENGTH];
	struct char_data *victim;
	int diff, athaco = 0, bthaco = 0, thac_diff, lev_diff, hit_diff, ac_diff;
	...
	lev_diff = (GET_LEVEL(victim) - GET_LEVEL(ch));
	if (IS_NPC(ch)) {
		athaco = GET_HITROLL(ch);
	} else {
		athaco = thaco((int) GET_CLASS(ch), (int) GET_LEVEL(ch));
	}
	if (IS_NPC(victim)) {
		bthaco = GET_HITROLL(victim);
	} else {
		bthaco = thaco((int) GET_CLASS(victim), (int) GET_LEVEL(victim));
	}
	thac_diff = (athaco - bthaco);
	hit_diff = (GET_HIT(victim) - GET_HIT(ch)) / 15;
	ac_diff = (((GET_AC(ch) + 100) - (GET_AC(victim) + 100)) / 10);
	diff = lev_diff + thac_diff + hit_diff + ac_diff;

	if (diff <= -100) {
		send_to_char(ch, "You would hate to dirty your weapon.\r\n");
	} else if (diff > -100 && diff <= -10) {
		send_to_char(ch, "You would definitely have the upper hand.\r\n");
		...
		send_to_char(ch, "May the force be with you!\r\n");
	} else if (diff > 20 && diff <= 30) {
		send_to_char(ch, "Is your will written yet?\r\n");
	} else if (diff > 30 && diff <= 50) {
		send_to_char(ch, "We will just go ahead and call the coroner.\r\n");
	} else {
		send_to_char(ch, "Bring out your dead!\r\n");
	}
}
...
ACMD(do_toggle) {
	...
	const struct {
		...
		{
			"autotitle", PRF_AUTOTITLE, 20, "Autotitle disabled.\r\n", "Autotitle enabled.\r\n"
		},
		{
			"nosport", PRF_NOSPORT, 0, "You can now hear sport.\r\n",
			"You are now deaf to sport.\r\n"
		},
		{
			"\n", 0, -1, "\n", "\n" /* must be last */
		}
	...
	if (!*arg) {
		...
		send_to_char(ch,
				...
				ONOFF(PRF_FLAGGED(ch, PRF_NOSPORT)));
		return;
	}
	...
	if (*tog_messages[toggle].command == '\n' || tog_messages[toggle].min_level > GET_LEVEL(ch)) {
		send_to_char(ch, "You cannot toggle that!\r\n");
		return;
	}
	...
	switch (toggle) {
		...
		case SCMD_BUILDWALK:
			...
			if (PRF_FLAGGED(ch, PRF_BUILDWALK)) {
				...
				mudlog(CMP, GET_LEVEL(ch), TRUE, "OLC: %s turned buildwalk on.  Allowed zone %llu",
						GET_NAME(ch), GET_OLC_ZONE(ch));
			} else {
				mudlog(CMP, GET_LEVEL(ch), TRUE, "OLC: %s turned buildwalk off.  Allowed zone %llu",
						GET_NAME(ch), GET_OLC_ZONE(ch));
			}
			break;
		case SCMD_AFK:
		...
		case SCMD_WIMPY:
			if (!*arg2) {
				if (GET_WIMP_LEV(ch)) {
					...
					send_to_char(ch, "At the moment, you are not a wimp.  (sure, sure...)\r\n");
					return;
				}
			}
			if (isdigit(*arg2)) {
				if ((wimp_lev = atoi(arg2)) != 0) {
					if (wimp_lev < 0) {
						...
						send_to_char(ch, "That does not make much sense, now does it?\r\n");
					} else if (wimp_lev > (GET_MAX_HIT(ch) / 2)) {
						send_to_char(ch, "You cannot set your wimp level above half your hit points.\r\n");
					} else {
						send_to_char(ch, "Okay, you will wimp out if you drop below %d hit points.",
								wimp_lev);
						GET_WIMP_LEV(ch) = wimp_lev;
					}
				} else {
					send_to_char(ch, "Okay, you will now tough out fights to the bitter end.");
					GET_WIMP_LEV(ch) = 0;
				}
			} else {
		...
		case SCMD_PAGELENGTH:
			if (!*arg2) {
				send_to_char(ch, "Your current page length is set to %d lines.",
						GET_PAGE_LENGTH(ch));
		...
		case SCMD_SCREENWIDTH:
			if (!*arg2) {
				send_to_char(ch, "Your current screen width is set to %d characters.",
						GET_SCREEN_WIDTH(ch));
			} else if (is_number(arg2)) {
...
ACMD(do_whois) {
	...
	if (!(GET_LEVEL(victim) < LVL_IMMORT) || (GET_LEVEL(ch) >= GET_LEVEL(victim))) {
		...
		if (!got_from_file) {
			send_to_char(ch, "Last Logon: They are playing now!  (Idle %d Minutes)",
	...
	if (!got_from_file && victim->desc != NULL && GET_LEVEL(ch) >= LVL_GOD) {
		...
		send_to_char(ch, "Color:   %s\r\n", prot->pVariables[eMSDP_XTERM_256_COLORS]->ValueInt ?
				"Xterm" : (prot->pVariables[eMSDP_ANSI_COLORS]->ValueInt ? "Ansi" : "None"));
	...
	for (i = 0; i <= top_of_zone_table; i++) {    /* Go through the whole zone table */
		...
		if (show_zone) {
			...
			tmp_len = snprintf(buf + len, sizeof(buf) - len, "\tn(%3d) %s%-*s\tn %s%s\tn\r\n",
					++zcount, overlap ? QRED : QCYN, count_color_chars(zone_table[i].name) + 30,
...
ACMD(do_scan) {
	...
	bool found = FALSE;
	int range;
	int maxrange = 7, skillrange = 4, godrange = 6;
	...
	for (door = 0; door < DIR_COUNT; door++) {
		for (range = 1; range <= maxrange; range++) {
			if (world[scanned_room].dir_option[door] &&
				...
				if ((range >= skillrange && range < godrange && !_CAN_HIGH_EXIT(ch)) ||
						(range >= godrange && !_CAN_HIGHEST_EXIT(ch))) {
					continue;
				}
				...
				if (IS_DARK(scanned_room) && !CAN_SEE_IN_DARK(ch)) {
					if (world[scanned_room].people) {
						send_to_char(ch, "%s: It is too dark to see, but you can hear shuffling.\r\n", dirs[door]);
					} else {
	...
	if (!found) {
		send_to_char(ch, "You do not see anything nearby!\r\n");
	}
}
...
ACMD(do_search)	{
	...
	if (IS_AFFECTED(ch, AFF_BLIND)) {
		send_to_char(ch, "You are blind, you cannot see a damned thing!");
		return;
	}
		...
		if (EXIT(ch, door) && EXIT(ch, door)->to_room != NOWHERE) {
			if (IS_SET(EXIT(ch, door)->exit_info, EX_HIDDEN)) {
				...
				if (rand_number(1, 6) <= chance) {
					send_to_char(ch, "\r\nYou have found a secret door %s.\r\n", dirs[door]);
					REMOVE_BIT(world[ch->in_room].dir_option[door]->exit_info, EX_HIDDEN);
					TOGGLE_BIT(world[EXIT(ch, door)->to_room].dir_option[rev_dir[door]]->exit_info,
							EX_HIDDEN);
				}
			}
		}

...
* File: act.movement.c                                                          A CircleMUD/tbaMUD *
* Description: Movement commands, door handling, & sleep/rest/etc state.                           *
*                                                                                                  *
...
/**
 * Move a PC/NPC character from their current location to a new location. This is the standard
 * movement locomotion function that all normal walking movement by characters should be sent
 * through. This function also defines the move cost of normal locomotion as:
 * ( (move cost for source room) + (move cost for destination) ) / 2
 *
 * @pre Function assumes that ch has no master controlling character, that ch has no followers (in
 * 		other words followers won't be moved by this function) and that the direction traveled in is
 * 		one of the valid, enumerated direction.
 * @param ch The character structure to attempt to move.
 * @param dir The defined direction (NORTH, SOUTH, etc...) to attempt to move into.
 * @param need_specials_check If TRUE will cause
 * @retval int 1 for a successful move (ch is now in a new location) or 0 for a failed move (ch is
...
int do_simple_move(struct char_data *ch, int dir, int need_specials_check) {
	...
	* when checking for specials */
	...
	/*
	 * How many movement points are required to travel from was_in to going_to. We redefine this
	 * later when we need it.
	 */
	int need_movement = 0, same_room = 0, riding = 0, ridden_by = 0;
	...
	/*
	 * Begin checks that can prevent a character from leaving the was_in room. Future checks should
	 * be implemented within this section and return 0.
	 */
	/*--------------------------------------------------------------------------------------------*/
	/*
	 ...
	 * (ie. North is '1' in the command list, but NORTH is defined as '0').
	...
	/* If they are mounted, are they in the same room with their mount(ee)? */
	...
	if (ridden_by && same_room && (AFF_FLAGGED(ch, AFF_TAMED) || AFF_FLAGGED(ch, AFF_DOCILE))) {
		send_to_char(ch, "You have been tamed. Now act it!\r\n");
		return (0);
	}
	...
	if ((SECT(was_in) == SECT_FLYING) || (SECT(going_to) == SECT_FLYING)) {
		if (!has_flight(ch)) {
			send_to_char(ch, "What, do you think you are a bird?!\r\n");
			return (0);
		}
	}
	...
	/* Checks for going on a wall */
	if ((SECT(was_in) == SECT_WALL) || (SECT(going_to) == SECT_WALL)) {
		if (riding || ridden_by) {
			send_to_char(ch, "You cannot move on a wall while mounted.\r\n");
			return (0);
		}
		if (GET_SKILL(ch, SKILL_CLIMBING) >= rand_number(1, 96)) {
			send_to_char(ch, "You do not feel confident enough to move in that direction.\r\n");
			return (0);
		}
	}
	...
	if (ROOM_FLAGGED(was_in, ROOM_ATRIUM)) {
		if (!House_can_enter(ch, GET_ROOM_VNUM(going_to))) {
			send_to_char(ch, "That is private property -- no trespassing!\r\n");
			return (0);
		}
	}
	...
	if (ROOM_FLAGGED(going_to, ROOM_TUNNEL) && ((riding || ridden_by) ||
			num_pc_in_room(&(world[going_to])) >= CONFIG_TUNNEL_SIZE)) {
		if (CONFIG_TUNNEL_SIZE > 1) {
			send_to_char(ch, "There is not enough room for you to go there!\r\n");
		} else if (riding || ridden_by) {
			send_to_char(ch, "There is not enough room there, while mounted.\r\n");
		} else {
			send_to_char(ch, "There is not enough room there for more than one person!\r\n");
		}
		return (0);
	}
	...
	/* All checks passed, nothing will prevent movement now other than lack of move points. */
	/* move points needed is avg. move loss for src and destination sect type */
	need_movement = (movement_loss[SECT(was_in)] + movement_loss[SECT(going_to)]) / 2;

	if (ROOM_SNOW(ch->in_room) > 0) {
		need_movement += (ROOM_SNOW(ch->in_room) / 10);
	}
	...
	/* Does the ch have the necessary riding skills? */
	if (riding && GET_SKILL(ch, SKILL_RIDING) < rand_number(1, 91) - rand_number(-4, need_movement)) {
		...
		if (rand_number(1, 3) == 2) {
			send_to_char(ch, "You realize how you can improve your riding technique.\r\n");
	...
	if (riding) {
		if (!AFF_FLAGGED(RIDING(ch), AFF_SNEAK)) {
			if (AFF_FLAGGED(ch, AFF_SNEAK)) {
				...
				act(leave_message, TRUE, RIDING(ch), 0, 0, TO_ROOM);
			} else {
				snprintf(leave_message, sizeof(leave_message), "$n rides $N %s.", dirs[dir]);
				act(leave_message, TRUE, ch, 0, RIDING(ch), TO_NOTVICT);
			}
		}
	} else if (ridden_by) {
		if (!AFF_FLAGGED(ch, AFF_SNEAK)) {
			if (AFF_FLAGGED(RIDDEN_BY(ch), AFF_SNEAK)) {
				...
				snprintf(leave_message, sizeof(leave_message), "$n rides $N %s.", dirs[dir]);
				act(leave_message, TRUE, RIDDEN_BY(ch), 0, ch, TO_NOTVICT);
			}
		}
	 ...
	 * Assumptions: The character has already truly left the was_in room. If the entry trigger
	 * "prevents" movement into the room, it is the triggers job to provide a message to the
	 * original was_in room.
	 */
	...
	if (ROOM_FLAGGED(going_to, ROOM_DEATH)){
		if (GET_LEVEL(ch) < LVL_IMMORT) {
			mudlog(BRF, LVL_IMMORT, TRUE, "%s hit death trap #%llu (%s)", GET_NAME(ch),
					GET_ROOM_VNUM(going_to), world[going_to].name);
			death_cry(ch);
			extract_char(ch);
		}
		if (riding && GET_LEVEL(RIDING(ch)) < LVL_IMMORT) {
			mudlog(BRF, LVL_IMMORT, TRUE, "%s hit death trap #%llu (%s)", GET_NAME(RIDING(ch)),
					GET_ROOM_VNUM(going_to), world[going_to].name);
			death_cry(RIDING(ch));
			extract_char(RIDING(ch));
		}
		if (ridden_by && GET_LEVEL(RIDDEN_BY(ch)) < LVL_IMMORT) {
			mudlog(BRF, LVL_IMMORT, TRUE, "%s hit death trap #%llu (%s)", GET_NAME(RIDDEN_BY(ch)),
					GET_ROOM_VNUM(going_to), world[going_to].name);
	...
	/*
	 * At this point, the character is safe and in the room. Fire memory and greet triggers, check
	 * and see if the greet trigger prevents movement, and if so, move the player back to the
	 * previous room.
	 */
	...
	/*
	 * Only here is the move successful *and* complete. Return success for calling functions to
	 * handle post move operations.
	 */
	return (1);
}
...
static int find_door(struct char_data *ch, const char *type, char *dir, const char *cmdname) {
	...
	if (*dir) {			/* a direction was specified */
		...
		if (EXIT(ch, door)) {	/* Braces added according to indent. -gg */
			...
			send_to_char(ch, "I really do not see how you can %s anything there.\r\n", cmdname);
			return (-1);
		}
...
#define OPEN_DOOR(room, obj, door)	((obj) ? (REMOVE_BIT(GET_OBJ_VAL(obj, 1), CONT_CLOSED)) :\
			(REMOVE_BIT(EXITN(room, door)->exit_info, EX_CLOSED)))
#define CLOSE_DOOR(room, obj, door)	((obj) ? (SET_BIT(GET_OBJ_VAL(obj, 1), CONT_CLOSED)) :\
			(SET_BIT(EXITN(room, door)->exit_info, EX_CLOSED)))
#define LOCK_DOOR(room, obj, door)	((obj) ? (SET_BIT(GET_OBJ_VAL(obj, 1), CONT_LOCKED)) :\
			(SET_BIT(EXITN(room, door)->exit_info, EX_LOCKED)))
#define UNLOCK_DOOR(room, obj, door)	((obj) ? (REMOVE_BIT(GET_OBJ_VAL(obj, 1), CONT_LOCKED)) :\
			(REMOVE_BIT(EXITN(room, door)->exit_info, EX_LOCKED)))
#define TOGGLE_LOCK(room, obj, door)	((obj) ? (TOGGLE_BIT(GET_OBJ_VAL(obj, 1), CONT_LOCKED)) :\
...
static void do_doorcmd(struct char_data *ch, struct obj_data *obj, int door, int scmd) {
	...
	/* Notify the room. */
	if (len < sizeof(buf)) {
		snprintf(buf + len, sizeof(buf) - len, "%s%s.", obj ? "" : "the ", obj ? "$p" :
...
static int ok_pick(struct char_data *ch, obj_vnum keynum, int pickproof, int scmd) {
	...
	if (keynum == NOTHING) {
		send_to_char(ch, "Odd - you cannot seem to find a keyhole.\r\n");
	} else if (pickproof) {
		send_to_char(ch, "It resists your attempts to pick it.\r\n");
	} else if (percent > skill_lvl) {
		send_to_char(ch, "You failed to pick the lock.\r\n");
		if (rand_number(1, 20) == 17) {
			send_to_char(ch, "You realize how you can improve your picklocking technique.\r\n");
			SET_SKILL(ch, SKILL_PICK_LOCK, (GET_SKILL(ch, SKILL_PICK_LOCK) + 1));
		}
	} else {
		return (1);
	}
...
#define DOOR_IS_OPENABLE(ch, obj, door)	((obj) ? ((GET_OBJ_TYPE(obj) == ITEM_CONTAINER) && \
			OBJVAL_FLAGGED(obj, CONT_CLOSEABLE)) : (EXIT_FLAGGED(EXIT(ch, door), EX_ISDOOR)))
#define DOOR_IS_OPEN(ch, obj, door)	((obj) ? (!OBJVAL_FLAGGED(obj, CONT_CLOSED)) : \
			(!EXIT_FLAGGED(EXIT(ch, door), EX_CLOSED)))
#define DOOR_IS_UNLOCKED(ch, obj, door)	((obj) ? (!OBJVAL_FLAGGED(obj, CONT_LOCKED)) : \
			(!EXIT_FLAGGED(EXIT(ch, door), EX_LOCKED)))
#define DOOR_IS_PICKPROOF(ch, obj, door)	((obj) ? (OBJVAL_FLAGGED(obj, CONT_PICKPROOF)) : \
...
ACMD(do_gen_door) {
	...
	if ((obj) || (door >= 0)) {
		...
		} else if (!has_key(ch, keynum) && (GET_LEVEL(ch) < LVL_GOD) && ((subcmd == SCMD_LOCK) ||
				(subcmd == SCMD_UNLOCK))) {
			send_to_char(ch, "You do not seem to have the proper key.\r\n");
...
ACMD(do_enter) {
	...
	if (*buf) {			/* an argument was supplied, search for door
		...
		send_to_char(ch, "You cannot seem to find anything to enter.\r\n");
	}
}
		...
		case POS_STANDING:
			if (found == 0) {
				...
				if (GET_OBJ_TYPE(furniture) != ITEM_FURNITURE) {
					send_to_char(ch, "You cannot sit on that!\r\n");
					return;
				} else if (GET_OBJ_VAL(furniture, 1) > GET_OBJ_VAL(furniture, 0)) {
					/* Val 1 is current number sitting, 0 is max in sitting. */
					act("$p looks like it is all full.", TRUE, ch, furniture, 0, TO_CHAR);
					log("SYSERR: Furniture %llu holding too many people.", GET_OBJ_VNUM(furniture));
					return;
				} else if (GET_OBJ_VAL(furniture, 1) == GET_OBJ_VAL(furniture, 0)) {
			...
			break;
		case POS_SITTING:
			send_to_char(ch, "You are sitting already.\r\n");
			break;
		case POS_RESTING:
...
ACMD(do_wake) {
	...
	if (AFF_FLAGGED(ch, AFF_SLEEP)) {
		send_to_char(ch, "You cannot wake up!\r\n");
...
ACMD(do_mount) {
	...
	if (!*arg) {
		...
		send_to_char(ch, "You cannot mount that!\r\n");
		return;
	...
	mount_char(ch, vict);
	if (IS_NPC(vict) && !(AFF_FLAGGED(vict, AFF_TAMED) ||AFF_FLAGGED(vict, AFF_DOCILE)) &&
			GET_SKILL(ch, SKILL_MOUNT) <= rand_number(1, 101)) {
...
ACMD(do_dismount) {
	if (!RIDING(ch)) {
		send_to_char(ch, "You are not even riding anything.\r\n");
		return;
	} else if (SECT(ch->in_room) == SECT_WATER_NOSWIM && !has_boat(ch)) {
		send_to_char(ch, "Yeah, right, and then drown...\r\n");
		return;
	}
...
ACMD(do_buck) {
	if (!RIDDEN_BY(ch)) {
		send_to_char(ch, "You are not even being ridden!\r\n");
		return;
	} else if (AFF_FLAGGED(ch, AFF_TAMED) || AFF_FLAGGED(ch, AFF_DOCILE)) {
		send_to_char(ch, "But you are tamed!\r\n");
		return;
	}
...
ACMD(do_tame) {
	...
	if (!*arg) {
		...
		send_to_char(ch, "You cannot do that to them.\r\n");
		return;
	} else if (!GET_SKILL(ch, SKILL_TAME)) {
		send_to_char(ch, "You do not even know how to tame something.\r\n");
		return;
	} else if (!IS_NPC(vict)) {
		send_to_char(ch, "You cannot do that.\r\n");
		return;
	} else if (AFF_FLAGGED(vict, AFF_DOCILE) || AFF_FLAGGED(vict, AFF_TAMED)){

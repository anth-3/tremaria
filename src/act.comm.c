...
static int is_tell_ok(struct char_data *ch, struct char_data *vict) {
	if (!ch) {
		...
		send_to_char(ch, "You cannot tell other people while you have notell on.\r\n");
...
ACMD(do_tell) {
	...
	if (!*buf || !*buf2) {
		send_to_char(ch, "Who do you wish to tell, and what??\r\n");
...
ACMD(do_spec_comm) {
	...
	if (!*buf || !*buf2) {
		...
		send_to_char(ch, "You cannot get your mouth close enough to your ear...\r\n");
	} else {
...
ACMD(do_write) {
	...
	if (*penname) {
		...
		if (!GET_EQ(ch, WEAR_HOLD)) {
			send_to_char(ch, "You cannot write with %s %s alone.\r\n", AN(papername), papername);
			return;
		}
	...
	if (GET_OBJ_TYPE(pen) != ITEM_PEN) {
		...
		act("You cannot write on $p.", FALSE, ch, paper, 0, TO_CHAR);
	} else {
		char *backstr = NULL;
		...
		if (paper->action_description) {
			...
			send_to_char(ch, "There is something written on it already:\r\n");
...
>>>>>>> 6e935a44b17fdc12df2002178d34ed6a13fad3ab
	...
	if (subcmd == SCMD_HOLLER) {
		if (GET_MOVE(ch) < CONFIG_HOLLER_MOVE_COST) {
			send_to_char(ch, "You are too exhausted to holler.\r\n");
			return;
		} else {
	...
	for (i = descriptor_list; i; i = i->next) {
		...
		snprintf(buf2, sizeof(buf2), "%s%s%s", (COLOR_LEV(i->character) >= C_NRM) ? color_on : "",
				buf1, KNRM);
...
ACMD(do_qcomm) {
	if (!PRF_FLAGGED(ch, PRF_QUEST)) {
		send_to_char(ch, "You are not even part of the quest!\r\n");
		return;
	}
	skip_spaces(&argument);
...
>>>>>>> 6e935a44b17fdc12df2002178d34ed6a13fad3ab
		...
		if (subcmd == SCMD_QSAY) {
			...
			mudlog(CMP, MAX(LVL_BUILDER, GET_INVIS_LEV(ch)), TRUE,
					"(GC) %s qechoed: %s", GET_NAME(ch), argument);
		}
		for (i = descriptor_list; i; i = i->next) {

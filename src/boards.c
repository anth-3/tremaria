...
* File: boards.c                                                                A CircleMUD/tbaMUD *
* Description: Handling of multiple bulletin boards.                                               *
*                                                                                                  *
* All rights reserved.  See license for complete information.                                      *
*                                                                                                  *
* Copyright (C) 1993, 94 by the Trustees of the Johns Hopkins University                           *
* CircleMUD is based on DikuMUD, Copyright (C) 1990, 1991.                                         *
* Last updated: 31 March 2014 -- anth*3                                                            *
***************************************************************************************************/
...
/*
 * Format: vnum, read lvl, write lvl, remove lvl, filename, 0 at end. Be sure to also change
 * NUM_OF_BOARDS in board.h.
 */
...
int board_write_message(int board_type, struct char_data *ch, char *arg, struct obj_data *board) {
	...
	if (GET_LEVEL(ch) < WRITE_LVL(board_type)) {
		send_to_char(ch, "You are not of a high enough level to write on this board.\r\n");
		return (1);
	}
	...
	if (!*arg) {
		send_to_char(ch, "All messages must have a headline!\r\n");
		return (1);
	}
	ct = time(0);
...
int board_show_board(int board_type, struct char_data *ch, char *arg, struct obj_data *board) {
	...
	if (GET_LEVEL(ch) < READ_LVL(board_type)) {
		send_to_char(ch, "You try but fail to understand the words of those of a higher level than you.\r\n");
		return (1);
	}
...
int board_display_msg(int board_type, struct char_data *ch, char *arg, struct obj_data *board) {
	...
	if (GET_LEVEL(ch) < READ_LVL(board_type)) {
		send_to_char(ch, "You try but fail to understand the higher level words.\r\n");
		return (1);
	}
	if (!num_of_msgs[board_type]) {
...
int board_remove_msg(int board_type, struct char_data *ch, char *arg, struct obj_data *board) {
	...
	if (GET_LEVEL(ch) < REMOVE_LVL(board_type) && !(strstr(MSG_HEADING(board_type, ind), buf))) {
		send_to_char(ch, "You are not of a high enough level to remove other people's messages.\r\n");
		return (1);
	}
	if (GET_LEVEL(ch) < MSG_LEVEL(board_type, ind)) {
		send_to_char(ch, "You can't remove a message with a higher level than yourself.\r\n");
		return (1);
	}

...
* File: handler.c                                                               A CircleMUD/tbaMUD *
* Description: Internal funcs: moving and finding chars/objs.                                      *
*                                                                                                  *
...
/*
 * This updates a character by subtracting everything he is affected by restoring original
 * abilities, and then affecting all again.
 */
void affect_total(struct char_data *ch) {
	...
	for (i = 0; i < NUM_WEARS; i++) {
		if (GET_EQ(ch, i)) {
			for (j = 0; j < MAX_OBJ_AFFECT; j++) {
				affect_modify_ar(ch, GET_EQ(ch, i)->affected[j].location,
						GET_EQ(ch, i)->affected[j].modifier, GET_OBJ_AFFECT(GET_EQ(ch, i)), TRUE);
			}
		}
	}
	for (af = ch->affected; af; af = af->next) {
...
/*
 * Remove an affected_type structure from a char (called when duration reaches zero). Pointer *af
 * must never be NIL!  Frees mem and calls affect_location_apply
 */
...
/* Return TRUE if a char is affected by a spell (SPELL_XXX), FALSE indicates not affected. */
...
void extract_char_final(struct char_data *ch) {
	...
	/*
	 * We're booting the character of someone who has switched so first we need to stuff them back
	 * into their own body.  This will set ch->desc we're checking below this loop to the proper
	 * value.
	 */
	...
	if (ch->desc) {
		/*
		 * This time we're extracting the body someone has switched into (not the body of someone
		 * switching as above) so we need to put the switcher back to their own body. If this body
		 * is not possessed, the owner won't have a body after the removal so dump them to the main
		 * menu.
		 */
		if (ch->desc->original) {
			...
			/*
			 * Now we boot anybody trying to log in with the same character, to help guard against
			 * duping.  CON_DISCONNECT is used to close a descriptor without extracting the
			 * d->character associated with it, for being link-dead, so we want CON_CLOSE to clean
			 * everything up. If we're here, we know it's a player so no IS_NPC check required.
			 */
			for (d = descriptor_list; d; d = d->next) {
				if (d == ch->desc) {
					continue;
	...
	for (temp = character_list; temp; temp = temp->next) {
		...
		/*
		 * If "temp" has allocated memory data and our ch is a PC, forget the extracted character
		 * (if he/she is remembered)
		 */
...
/*
 * Why do we do this? Because trying to iterate over the character list with 'ch = ch->next' does
 * bad things if the current character happens to die. The trivial workaround of 'vict = next_vict'
 * doesn't work if the _next_ person in the list gets killed, for example, by an area spell. Why do
 * we leave them on the character_list? Because code doing 'vict = vict->next' would get really
 * confused otherwise.
 */
...
/*
 * I'm not particularly pleased with the MOB/PLR hoops that have to be jumped through but it hardly
 * calls for a completely new variable. Ideally it would be its own list, but that would change the
 * '->next' pointer, potentially confusing some code. -gg This doesn't handle recursive extractions.
 */
void extract_pending_chars(void) {
	...
	if (extractions_pending > 0) {
		log("SYSERR: Could not find %d extractions as counted.", extractions_pending);
	}
	extractions_pending = 0;
}

/*
 * Here follows high-level versions of some earlier routines, ie functions which incorporate the
 * actual player-data
 */
...
struct obj_data *create_money(int amount) {
	...
	if (amount == 1) {
		...
		new_descr->description = strdup("It is just one miserable little gold coin.");
	} else {
 ...
 *
 * The routine used to return a pointer to the next word in *arg (just like the one_argument
 * routine), but now it returns an integer that describes what it filled in.
 */
int generic_find(char *arg, bitvector_t bitvector, struct char_data *ch, struct char_data **tar_ch,
 ...
 * If a character is mounted on something, we dismount them.  If someone is mounting our character,
 * then we dismount that someone. This is used for cleaning up after a mount is cancelled by
 ...
 * Sets _ch_ to mounting _mount_.  This does not make any checks what-so-ever to see if the _mount_
 * is mountable, etc.  That is left up to the calling function.  This does not present any messages,
 * either.
 */
